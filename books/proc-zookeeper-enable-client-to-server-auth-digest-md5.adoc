...
This procedure will show you how to enable authentication using the SASL DIGEST-MD5 mechanism between Zookeeper clients and Zookeeper.
...
The file can contain multiple users.
But only one additional user is required by Kafka brokers.
The recommended name for the Kafka user is `kafka`.
+
Following example shows the `Server` context for client-to-server authentication:
+
[source,subs=+quotes]
----
Server {
...
For example the configuration for a three node Zookeeper cluster should look like this:
+
[source,ini,subs=+quotes]
----
requireClientAuthScheme=sasl
