...
----
+
If you're going to throttle replication you can also pass the `--throttle` option with an inter-broker throttled rate in bytes per second. For example:
+
[source,shell,subs=+quotes]
----
...
. The reassignment has finished when the `--verify` command reports each of the the partitions being moved as completed successfully. 
