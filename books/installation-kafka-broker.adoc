...
version of Kafka - for example `2.12` and `1.0.0`)
+
[source]
----
...
and does not change over time. The broker id is configured in the broker properties file:
...
When a producer or consumer tries to send or receive messages to / from a topic which does not exist, Kafka will, by default,
...
`delete.topic.enable` property, which is set to `true` by default (that is, deleting topics is possible). When this property is set to
