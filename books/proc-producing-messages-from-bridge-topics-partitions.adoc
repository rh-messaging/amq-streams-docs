...
Next, produce messages to a topic in JSON format by using the link:https://strimzi.io/docs/bridge/latest/#_send[topics^] endpoint. You can specify destination partitions for messages in the request body, as shown here. The link:https://strimzi.io/docs/bridge/latest/#_sendtopartition[partitions^] endpoint provides an alternative method for specifying a single destination partition for all messages as a path parameter.

.Procedure

. Create a Kafka topic with three partitions by using the `kafka-topics.sh` tool:
+
[source,shell,subs=attributes+]
----
bin/kafka-topics.sh --zookeeper <ZookeeperAddress>:2181 --create --topic bridge-quickstart-topic --partitions 3 --replication-factor 1 --config retention.ms=7200000 --config segment.bytes=1073741824
----
...
[source,shell,subs=attributes+]
----
bin/kafka-topics.sh --zookeeper  <ZookeeperAddress>:2181 --describe --topic bridge-quickstart-topic
----
