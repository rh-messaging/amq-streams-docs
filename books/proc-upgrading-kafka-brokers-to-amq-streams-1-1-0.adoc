...
= Upgrading Kafka brokers to {ProductName} 1.1.x

To upgrade to {ProductName} 1.1.x on the host machine, download and extract the *{ProductName} 1.1.x* archive from the Red Hat Customer Portal. 

NOTE: This procedure is part of the xref:assembly-upgrade-1-1-0-{context}[Upgrading an {ProductName} cluster from 1.0.0 to 1.1.x] chapter. For initial installation instructions, see xref:proc-installing-amq-streams-{context}[Installing {ProductName}].
...
----
+
NOTE: The Kafka broker will start using the version 1.1.x binaries.
